...
async function checkHasRolePermission(userId, roomName) {
  try {
    await axios({
...
async function checkUserIsModForEmoji(userId, room) {
...
export default async function roomEmojiSettings(req, res) {
  const { locals } = res;
  ...
  try {
    ...
    if (locals.user) {
      ...
      locals.userIsMod = await checkUserIsModForEmoji(String(locals.user._id), locals.room);
    }
  ...
  return res.render('room/settings/emoji');
}
