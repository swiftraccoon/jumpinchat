...
vi.mock('../../../utils/audioLevel', () => ({ default: vi.fn() }));
...
function fixtures() {
  const track = { enabled: true }; const stream = { getAudioTracks: () => [track] }; const detector = { on: vi.fn(), stop: vi.fn() }; createAudioLevelMonitor.mockReturnValue(detector);
